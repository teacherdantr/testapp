...
export default function TestPage() {
  ...
  if (pageState === TestPageState.PasswordPrompt && studentViewData) { // Use studentViewData for password check
    return <PasswordPrompt open={true} onVerify={handlePasswordVerify} error={passwordError} onOpenChange={() => { if (pageStateRef.current === TestPageState.PasswordPrompt) router.push('/select-test'); }}/>;
  }

  if (pageState === TestPageState.UserIdPrompt) {
    return <UserIdPrompt open={true} onIdentifierSubmit={handleUserIdSubmit} onOpenChange={() => { if (pageStateRef.current === TestPageState.UserIdPrompt) router.push('/select-test'); }} />;
  }

  if (pageState === TestPageState.ModeSelection) {
    return <ModeSelectionPrompt open={true} onModeSelect={handleModeSelect} onOpenChange={() => { if (pageStateRef.current === TestPageState.ModeSelection) router.push('/select-test'); }} />;
  }
          ...
          <div className="my-6 p-4 bg-card border rounded-lg shadow-sm">
            ...
            <div className="w-full bg-muted rounded-full h-6 overflow-hidden relative border">
              <div 
                ...
                style={{ width: `${(currentQuestionIndex / (activeQuestions.length -1)) * 100}%` }}
              >
              </div>
              <div 
                className="absolute top-0 h-full flex items-center transition-all duration-300 ease-out"
                style={{ left: `calc(${(currentQuestionIndex / (activeQuestions.length - 1)) * 100}% - 12px)` }} 
              >
